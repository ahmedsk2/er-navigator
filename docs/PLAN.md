...
- **What did not happen:** the three slices that would have built the spec (12A the code, 12B the demo instance, 12C the documents) never ran, so none of the above is in the app yet and `demo-nav.towardpcc.com` does not exist — no DNS record, no Coolify project, no application, nothing created or changed on the host. The spec, its review and its six corrections are the deliverable; the next session builds against them. Live at `fc5b33722aa3a569` (commit `7904624`), the same behaviour a nurse saw at the end of Phase 11 plus the mark on paper.
...
| Email | nodemailer over SMTP | Same. `navigator@towardpcc.com` through that mailbox's OWN SMTP host, port, username and password from Coolify env vars, exactly as a mail client would hold them; no relay (Ahmed, 9 September). The transport refuses to fall back to cleartext on 587 (`requireTLS`) and never negotiates below TLS 1.2 | The `towardpcc.com` SPF (`-all`) and DMARC (`p=reject`, strict alignment) records are guarded by another product's ADR and are not touched; DMARC passes on the provider's DKIM signature, so the only DNS change is the provider's DKIM selector record |
...
| Dependency overrides | Not specified | `pnpm.overrides` pins `deepmerge-ts ^8`, `mysql2 ^3.23.1` and `uuid ^11.1.1` (all transitive, none reachable from app code); Dependabot keeps them current | Cleared `pnpm audit` at moderate and above without waiting for prisma 8 or exceljs to update; the migration path was re-proven on an empty database after the `deepmerge-ts` major (changelog P7.8) |
...
Ahmed asked, at the Phase 11 gate, what is left before staff can try this themselves and before real patients go on it, and for the part that is Claude's to be built. A twelve-agent read-only audit produced the list (50 items, ranked by what would go wrong first); Ahmed chose the hosted demo copy on its own subdomain over a local one. `docs/specs/phase12-go-live-readiness.md` is the build, in three slices: 12A the code (the instance banner, the must-change-password gate, the MRN hints, the three audit actions, the alert retry, the demo seed and the hygiene items), 12B the demo instance at `demo-nav.towardpcc.com`, 12C the documents (the runbook's demo section, a one-page nurse guide, a fifteen-minute demo script, and the plan and runbook corrections the audit found).

Recipe as run: one workflow. Fable wrote the brief and held the gate; Opus did everything else — the code map, the spec, the spec review (four lenses, three refuters per finding) and its six fixes, each proven by `tests/unit/phase12-spec.test.ts` reading the document against `app/globals.css`, `docker-compose.production.yml` and `worker/alerts.ts`; one deploy. The slice step then failed: the merge was handed two branch names that were the literal string `null` and there were no branches, no worktrees and no commits to merge, so 12A, 12B and 12C are all still to do. Nothing was provisioned on the host and production was not touched. Phase 13 starts by building 12A against the corrected spec.
