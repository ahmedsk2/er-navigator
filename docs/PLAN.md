...
- Still with Ahmed after this phase: whether "Not decided yet" should be renamed once nurses have used it, since two of the eight outcomes are none of the three pathways and it is therefore a real state and not only a default; and everything Phase 13 left with him, unchanged.
...
Verified on the lead database at the close: typecheck, lint, 1,188 unit and database tests in 66 files, the production build, 265 Playwright tests at 390 x 844 and 1280 x 800 (63 project skips), 15 in `tests/instance`, 7 in `tests/demo` on a reset demo database, the app-role privilege guard, and `prisma migrate diff --from-config-datasource --to-schema prisma/schema.prisma --exit-code` reporting no difference once the migration is applied. Gate captures in `design/screens/phase15-*.png`.

Recipe as run: Fable takes Ahmed's request and holds the gate; Opus does everything else in one session on `main` in the main checkout, the spec first and then tests first per item, one commit per coherent item, then the whole chain.
