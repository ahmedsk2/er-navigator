...
**Delivered in Phase 16 (13 September; Ahmed, the day the host script let him back into the admin account: "create a very simple reset admin to email page and link in the log in page using the minimal tokens")**

- A self-service way back in. **Forgot your password?** under the sign-in form goes to `/forgot`, which takes a username and always answers the same sentence — "If that account has an email, a link is on its way. It works for 30 minutes." — whether the account exists, is deactivated, has no address, has already asked three times this hour, or is genuinely being sent a link. A public form that answered differently would be a way to ask the application which members of staff exist, which is the discipline the sign-in form has had since Phase 1.
- The link is one-time and short-lived. 32 random bytes, base64url, in the URL and in nothing else; only their sha256 is stored, the digests are compared in constant time, the row is spent the moment it is used, it dies after thirty minutes, and asking again kills the earlier link. At most three links an hour per account, and a fourth is silently ignored. `/reset` shows two password fields under the twelve-character rule `/account` already enforces, says nothing about the account behind the link, and gives one answer for a token that cannot be spent, so "already used" and "no such link" cannot be told apart.
- The app does not send the mail; the worker does. `/forgot` appends a row to a small `Outbox` table and the alerts worker drains it every 20 seconds on its own timer, marking `sentAt`, or `attempts` and `lastError`. The 5-minute alert cycle, the heartbeat and the push monitor are untouched, so a mail server that is down can never make that container report unhealthy. With `SMTP_HOST` empty the worker logs the message the way it logs an alert and stamps the row sent, which is exactly what the demo instance does: the flow is fully demonstrable there and nothing leaves the host.
- Setting the password is not a second implementation. It goes through the store port P15.63 built, so one place in this application sets a password hash, clears the lock and the failure count, writes the `user.password` audit row and deletes every session. Two things differ, both carried on the origin: `mustChangePassword` stays **false**, because the user chose this password themselves, and the audit row says `via: 'email-reset'`. Three audit rows in all — `auth.reset.requested` (payload `{ requested: true }`, and nothing at all is written when nothing is issued), `user.password`, and `auth.fail` for a token that could not be spent — and no token value in any of them, or in any log line.
- One additive migration, two tables, nothing else touched: no column of any existing table, no enum, no taxonomy name, no permission, no figure in the deck or the Adaa form, no change to the Content-Security-Policy, no new environment variable on the app or on the worker, and no new dependency. VIEWER accounts may use it, because it is their own password. An account with **no email address** cannot, and is told nothing different; the host script (P15.63) stays the answer for that, and the runbook says so.
- The chain the phase closes on: typecheck, lint, 1,268 unit and database tests in 73 files, the production build, 277 Playwright tests at 390 x 844 and 1280 x 800 with 63 project skips and none flaky at two workers, 16 in `tests/instance`, 7 in `tests/demo`, the app-role privilege guard extended to `Outbox`, and no schema drift. Gate captures in `design/screens/phase16-*.png`.

**Delivered in Phase 15 (12 September; Ahmed, after the Phase 14 gate: "some are suitable for discharged patients and some for admitted patients only and others for referred patients to other facility ... Hide them and show what is related based on patient trajectory")**

- The case sheet asks the department's own question first. A **Patient trajectory** chip row at the top of the Patient journey block, on the new-case form and on an open case alike: **Not decided yet** (the default, stored NULL), **Discharge**, **Admission**, **Transfer to another facility**. While no outcome has been chosen the block follows it: an admission is asked for the admission order, the bed request and the bed; a transfer for the fax, the acceptance and the RCC; a discharge for neither. Once an outcome is chosen the Phase 13 rules govern as they always have.
- The other blocks follow it. **Referral out** opens for a transfer trajectory as well as for a referral reason and the TRANSFERRED outcome; the **Ward** chips and the isolation room appear for an admission pathway as well as for ADMITTED, so a bed can be recorded the hour it is given; and the **Final disposition** list is narrowed to the outcomes that pathway can end in, with a plain **Show all outcomes** control so every one of the eight stays reachable and the outcome a case already carries is always offered. The delay stages and reasons are not filtered: every stage can delay any pathway.
- Nothing recorded is dropped, and nothing new is required. A time entered under another pathway stays saved and is named on the Phase 13 "Also recorded" line; the trajectory is never required, never inferred from the disposition, and makes no journey time mandatory — what a case must record before it can be closed is still decided by its outcome alone. One additive migration, one enum and one nullable column, no data touched, no taxonomy name changed, no change to the permission matrix, and no figure in the weekly deck, the Adaa form or the printed report moves.
- The trajectory travels with the case: a row on the case summary sheet above the outcome, the first part of the handover sheet's per-case line, and one column before `Disposition` on the workbook's Cases sheet. Every seeded demo case carries one that agrees with its outcome, the demo kit taps it, and both guides are rewritten with the drift guard extended.
- The three questions Phase 14 left open are closed: the summary panel counts one escalation once, the handover sheet cuts the delay action at 200 characters, and the unused `addCaseUpdate` server action is retired (the service function, its permission check and its tests stay). The chain the phase left green: 1,188 unit and database tests in 66 files, 265 Playwright tests at 390 x 844 and 1280 x 800 with 63 project skips, 15 in `tests/instance`, 7 in `tests/demo`, the privilege guard, the build and no schema drift.
- Reviewed with Phase 14 in one round, and deployed. Four lenses over the combined diff of the two phases (correctness, security, UX and documents, tests) with three refuters per finding: fourteen findings raised, six confirmed, eight refuted and not carried, and not one of the six a product defect. Two were test cover that could have been deleted with the suite green, the OTHER-outcome guard in `visibleJourneyFields` and the handover sheet's action row, each now pinned by an assertion that failed first; four were sentences in documents that had drifted off the screen they describe, the nurse guide still sending a navigator to the composer Phase 14 removed and missing the Phase 13 stage rule, the demo script still saying "add an update", and three documents naming the printed report as a place the case timeline appears, which it never has. The drift guard in `tests/unit/phase12-spec.test.ts` gained a case for each document finding. The chain both phases close on: typecheck, lint, 1,192 unit and database tests in 66 files, the production build, 267 Playwright tests at 390 x 844 and 1280 x 800 with 63 project skips and none flaky at two workers, 15 in `tests/instance`, 7 in `tests/demo`, the app-role privilege guard and no schema drift, with both additive migrations applied on production and on the demo. Both phases close at commit `41f744e` ([ERN-P15.60]), fingerprint `397ca55234709fe8`, with `/api/ready` returning `ready` and `verify-live` green. The fingerprint of the live build is that of the latest docs commit; the numbers here name the close commit.
- The demo instance was redeployed by hand to the same build, because it does not follow a push, and reseeded, because its cases predated the trajectory: it holds ten cases, every one carrying a trajectory that agrees with its outcome and all three pathways on the board, four of them resolved with a delay action and an escalation answer, and four accounts. The audit rows of the earlier seed stay behind, because `AuditLog` is append-only.
- One product point for Ahmed, raised by the review round and deliberately not built, because it is his call and not a defect: three of the weekly deck's action categories, PRO / social work, forced / safety admission and DAMA management, could only ever arrive as a tag on a `CaseUpdate`, and Phase 14 took the composer and its tag chips off the case page. The four that survive are the ones the case's own fields imply. So those three rows can no longer be recorded at all and will read zero until he decides whether the Resolve block should carry an action-type chip row beside "What was done to solve the delay".
- Still with Ahmed after these two phases: whether the Resolve block should carry an action-type chip row, without which three of the deck's action categories can no longer be recorded; whether the Case management block stays or goes, which is his own decision B of Phase 8b and the one thing he asked about rather than asked for during Phase 14; the recorded Left ED deviation, which keeps a departure recordable for the two outcomes that belong to no pathway; whether "Not decided yet" is the right label on the chip once nurses have used it, since it is a real state and not only a default; whether the `Trajectory` column belongs immediately before `Disposition` on the workbook's Cases sheet; and everything Phase 13 and Phase 12 left with him, unchanged.

**Delivered in Phase 14 (12 September; Ahmed, after working the Phase 13 sheet: hide the timeline, hide the updates section, and ask instead what was done about the delay and whether it was escalated)**

- The case page lost two sections and gained two answers. The server-rendered Timeline and the whole Updates section (the composer with its action tag, the Add button and the list) are off `/cases/[id]`, and the "Jump to" strip is Delay, Teams, Tests, Times, Resolve. First in the Resolve block, above "Final disposition": **What was done to solve the delay**, a free-text box with dictation, its own cap and the "MRN only, no names" hint, and **Escalated to medical director** as Yes / No chips that clear on a second tap. Both editable before and after the case is resolved, neither ever required.
- Nothing recorded is lost, and the record stays append-only. A save or a resolve that changes the box appends one `CaseUpdate` carrying the same text, authored by whoever pressed Save, tagged as a leadership escalation on the move to Yes; a second text appends a second row and never rewrites the first. So the weekly deck's history, the board's "Updated 3h ago" and the QCH Comments column all keep working with the composer gone, and the timeline is still in the case summary sheet and on the handover sheet.
- The figures and the sheets follow: "actions documented" counts the escalation answer beside the medical-admin timestamp, and counts a non-empty "what was done" text as an untagged documented action; the workbook's Cases sheet ends with the two new columns; the case summary sheet and the handover sheet show both when the case carries them; the printed report changes only through the KPI.
- One additive migration, two nullable columns, no data touched, no taxonomy name changed, no change to the permission matrix. The demo seed gives every resolved case a "what was done" text and the update a real save would have mirrored from it, and both guides are rewritten with the drift guard extended to catch the next drift. The chain the phase left green: 1,152 unit and database tests in 66 files, 249 Playwright tests at 390 x 844 and 1280 x 800 with 63 project skips, 15 in `tests/instance`, 7 in `tests/demo`, the privilege guard, the build and no schema drift.
- Reviewed with Phase 15 and live in the same build. The two phases were built a day apart on `main` and reviewed together in one four-lens round over the combined diff, with three refuters per finding: fourteen raised, six confirmed, none of them a product defect, and the numbers, the fixes and the product point the round left with Ahmed are in the Phase 15 block above. Both phases close at commit `41f744e` ([ERN-P15.60]), fingerprint `397ca55234709fe8`, with `/api/ready` returning `ready` and `verify-live` green. The fingerprint of the live build is that of the latest docs commit; the numbers here name the close commit.
- Still with Ahmed after this phase: the two questions this one leaves open, namely whether "Documented actions" on the summary panel should read an escalation once rather than twice when the tagged row it counts is the one the chip itself produced, and whether the delay-action text belongs on the handover sheet at full length; and everything Phase 13 left with him, unchanged. Both were answered inside Phase 15, one commit each, alongside the retirement of the unused `addCaseUpdate` server action; what is still with Ahmed is in the Phase 15 block above.
...
- Reviewed and deployed. A four-lens review of the whole diff with three refuters per finding raised fourteen findings and confirmed five, plus one cheap note, every one fixed with a test that failed first: the shift was being stamped from the navigator's last shift behind a closed section, so a wrong one perpetuated itself unseen while the dashboard's split and the export column read it, and it is back in the identity block as a chip row; a case resolved under the old rule could be reopened into a state it could not leave, so "Reopen case" now asks for confirmation over a sentence naming the times that outcome will owe; the demo seed wrote four times and stopped, so its own resolved cases were rows the app would refuse to resolve; both guides still described the old sheet; the last fixed sleep in the demo kit is gone; and the closed "More to record" toggle, which reveals nothing recorded, no longer prints. The chain the phase left green: 1,126 unit and database tests in 66 files, 237 Playwright tests at 390 x 844 and 1280 x 800 with 63 project skips, 15 in `tests/instance`, 7 in `tests/demo`, the privilege guard, the build and no schema drift. There is no migration. The phase closes at commit `6a9175c` ([ERN-P13.60]), fingerprint `38be3187a0f74e5e`, with `/api/ready` returning `ready` and `verify-live` green. The fingerprint of the live build is that of the latest docs commit; the numbers here name the close commit.
- The demo instance was redeployed by hand to the same build, because it does not follow a push, and its data was reset by a scoped delete and reseeded, because its resolved cases predated the rule. It holds ten cases and four accounts again, every resolved one satisfying what its outcome owes; the audit rows of the earlier seed stay behind, because `AuditLog` is append-only.
- Still with Ahmed after this phase: the three questions Phase 13 leaves open, which are wording and policy rather than code (whether the block should be called "Patient journey"; whether a death should keep "Disposition decided" optional; whether "Other" should hide nothing beyond what the stage rules already hide); and everything Phase 12 left with him, unchanged: the first admin's password, the repository still public, the SMTP test send and the `Authentication-Results` check, the staff roster and their accounts in Admin, the hospital authorisation before any real MRN goes off-site, whether the ward Wi-Fi reaches the demo subdomain and behind how many addresses, the first sign-in on the demo, and the eight questions the Phase 12 spec leaves open.
...
### Phase 16: "Forgot your password?" (asked for on 13 September)

Ahmed was locked out of the admin account that morning and was let back in by `node reset-password.js` on the host (P15.63). His request the same day, in one sentence: "create a very simple reset admin to email page and link in the log in page using the minimal tokens." He is the owner and the only administrator, so the host script is the floor and not the path: this phase adds the path.

`docs/specs/phase16-forgot-password.md` is the spec: the exact sentence, the two tables, the outbox, the token rules, the contracts that stay and the fail-first test list.

**What changed on the screen.** One link, **Forgot your password?**, under the sign-in form. `/forgot` takes a username and answers the same sentence for every outcome. `/reset?token=…` takes a new password twice, under the twelve-character rule `/account` already enforces, and lands on `/login?reset=1`, which says "Password changed. Sign in." A link that is invalid, spent or expired shows one sentence and the way back to `/forgot`. Nothing else on any existing screen moves.

**Why there is an outbox.** The app has no SMTP transport in any request path and this phase does not give it one: a server action that opens a socket to a mail server blocks a nurse's form on that mail server, and a hospital host that is slow would turn "ask for a link" into a thirty-second spinner. So `/forgot` appends to a small `Outbox` table and the alerts worker — which already owns the mailer, the log-only mode and the retry discipline — drains it every 20 seconds on its own timer, with its own overlap guard, never touching the heartbeat and never calling the push monitor. A reset email is not a patient who has been waiting twelve hours, and it must never be the reason that container reports unhealthy. With `SMTP_HOST` empty the message is logged and the row is stamped sent, so the queue on an instance with no mail does not grow for ever; that is the demo instance, and it is what makes the flow demonstrable there with nothing leaving the host.

**The security, stated as the rules the tests assert.** 32 random bytes base64url in the link and only sha256 of them in the row; a constant-time compare of the digests; single use, stamped in the same transaction as the password write; thirty minutes; every earlier link of that account killed when a new one is issued; at most three an hour per account, a fourth silently ignored; five requests a minute per client IP on both pages, in their own bucket rather than the sign-in one, so five failed sign-ins do not stop the link being sent; one sentence for every outcome of `/forgot` and one for every bad token; no username on the reset page; audit rows for the request, the completion and the failure, with no token value in any of them or in any log line; and the Content-Security-Policy untouched, because these are server actions with no inline script. The threats deliberately accepted are written down in the spec: whoever can read the mailbox can take the account, which is what "reset by email" means, and the in-process rate limiter is per container as it has been since Phase 1.

**The data.** One additive migration, `20260913110000_password_reset_and_outbox`: `PasswordResetToken` (the shape `Session` uses, with `usedAt` meaning "cannot be used again", spent or superseded, which is what keeps it free of a delete path) and `Outbox` (append-only from the app; `prisma/sync-app-role.ts` revokes DELETE on it beside `Case`, `User` and `Alert`). No column of any existing table is added, renamed, retyped or backfilled, and neither table can hold a patient identifier.

**One thing the chain caught, and it was the right thing.** `tests/db/demo-seed.test.ts` migrates a schema of its own, which replays the Phase 1 privileges migration, whose GRANTs name `SCHEMA public` literally — so running the suite hands `ernav_app` DELETE back on every public table, and the narrowing that undoes it only listed the tables that existed when it was written. The new table walked straight into that gap and the assertion Phase 12 built for exactly this failed. `Outbox` is now in the narrowing list, in the list that test reads back, and in CI's privilege guard.

Verified on the lead database: typecheck, lint, 1,268 unit and database tests in 73 files, the production build, 277 Playwright tests at 390 x 844 and 1280 x 800 (63 project skips, none flaky at two workers), 16 in `tests/instance`, 7 in `tests/demo` on a reset demo database, the app-role privilege guard, and `prisma migrate diff --from-config-datasource --to-schema prisma/schema.prisma` reporting an empty migration once applied. Gate captures in `design/screens/phase16-*.png`.

Recipe as run: Fable wrote the brief and the design and holds the gate; Opus did everything else in one session on `main` in the main checkout — the spec from the brief, the build tests first with one commit per coherent item, the whole verification chain, the deploy check by fingerprint, the demo redeploy and the demo proof, and these documents.

### Phase 15: the patient trajectory (asked for on 12 September)

Ahmed, after the Phase 14 gate, in one sentence: "For patient journey time inputs, some are suitable for discharged patients and some for admitted patients only and others for referred patients to other facility like the fax sent at and accepted at and RCC transferred the patient at. Same goes for other categories in different labels. Hide them and show what is related based on patient trajectory."

Phase 13 already hid what a case cannot have, but only once the OUTCOME was chosen, which is at the end of the stay. For the ten hours before that the sheet either showed every chain or guessed from the delay stages. `docs/specs/phase15-trajectory.md` is the spec: the labels, the enum and column names, the visibility function's table, the disposition narrowing, the export column, the failing assertion each item starts from, a "contracts that stay" list, and one recorded deviation.

**What changed on the screen.** A **Patient trajectory** chip row at the top of the Patient journey block, on the new-case form and on an open case alike: **Not decided yet** (the default, stored NULL), **Discharge**, **Admission**, **Transfer to another facility**. While no disposition is chosen the block follows it — a discharge shows the six steps every patient has, an admission adds the order, the bed request and the bed, a transfer adds the fax, the acceptance and the RCC — and the Phase 13 stage implications stand down the moment a trajectory is set. Once a disposition is chosen the Phase 13 outcome rules govern exactly as before. The other blocks follow too: **Referral out** shows for a transfer trajectory as well as for a `requiresReferralNo` reason and the TRANSFERRED outcome, the **Ward** chips and the isolation room show for an admission pathway as well as for ADMITTED, and the **Final disposition** list is narrowed to the outcomes that pathway can end in, behind a plain **Show all outcomes** control so every one of the eight stays reachable. The delay stages and reasons are NOT filtered: every stage can delay any pathway.

**Why nothing is lost.** Changing the trajectory hides a step and never clears a column. A time entered under another pathway stays in the draft, is saved by "Save changes" and "Mark resolved", and is named on the Phase 13 "Also recorded" line, which needed no edit at all: it is defined as "recorded and not visible" and reads the same pure function. The narrowing has the same property — the outcome a case already carries is always offered, so a select can never be left without an option for its own value.

**One recorded deviation**, in item 3 of the spec. Fable's design put `Left ED` under each of the three trajectories and not in the undecided view; it stays in the undecided view here, because `LEFT_WITHOUT_BEING_SEEN` and `DECEASED` are neither a discharge nor an admission nor a transfer, and a patient who walked out could otherwise have no departure recorded at all — `/cases/new` has no Resolve block to record one in instead.

**Three Phase 14 follow-ups**, folded in, one commit each: the case summary panel counts one escalation once (the chip and the `CaseUpdate` the mirroring rule appended from it are the same escalation, and it read "×2"); the handover sheet cuts the delay action at 200 characters with an ellipsis, on the sheet only; and the unused `addCaseUpdate` server action is retired, keeping the service function, its permission check and its tests.

**The data.** One additive migration, `20260912130000_patient_trajectory`: `CREATE TYPE "Trajectory"` and `Case.trajectory Trajectory?`, nullable, no default, no backfill, no existing row read or written, and no `GRANT` needed. It is never required, never inferred, and nothing in `kpi.ts` reads it — a plan is not an action, and no figure in the deck or the Adaa form counts one. The taxonomy is untouched: `Trajectory` is a new vocabulary of three, in the way Phase 8b's answers and Phase 10's payers were.

**What the review round found.** One four-lens round covered Phases 14 and 15 together, over the combined diff (correctness, security, UX and documents, tests), with three refuters per finding: fourteen findings raised, six confirmed, eight refuted and not carried, and not one of the six a product defect. Two were test cover that could have been deleted with the whole suite green. The `!context.disposition` guard in `visibleJourneyFields` is reachable only by an OTHER-outcome case, the one outcome whose row reaches the trajectory rule at all, and nothing pinned it; one test does now. Every part of the handover sheet's `actionLine` was asserted in the unit test and none of the `[data-action-row]` that draws it, so the row could have left the sheet with CI green; it is read in a browser now, in print mode and at both viewports, because a sheet is paper and the paper is the same size whichever screen printed it. Four were sentences in documents that had drifted off the screen they describe: the nurse guide still sent a navigator on a red case to the Updates composer Phase 14 removed and had lost the Phase 13 stage rule for a case nobody has tapped a chip on; the demo script's handoff paragraph still said "add an update"; and three documents named the printed report as a place the case timeline appears, which it never has, `/report` being the dashboard's figures over a date range with no per-case section of any kind. Each fix landed with an assertion that failed first, and the drift guard in `tests/unit/phase12-spec.test.ts` gained a case for each document finding. The round also raised one product point that is Ahmed's call and not a defect, recorded at the end of the Phase 15 block in Section 0: with the update tag chips off the case page, three of the weekly deck's action categories can no longer be recorded by any code path.

Verified on the lead database at the close, after the review fixes: typecheck, lint, 1,192 unit and database tests in 66 files, the production build, 267 Playwright tests at 390 x 844 and 1280 x 800 (63 project skips, none flaky at two workers), 15 in `tests/instance`, 7 in `tests/demo` on a reset demo database, the app-role privilege guard, and `prisma migrate diff --from-config-datasource --to-schema prisma/schema.prisma --exit-code` reporting no difference once the migration is applied. Gate captures in `design/screens/phase15-*.png`, re-taken from the reviewed tree. Both migrations are applied on production and on the demo, and the demo instance was redeployed by hand to the same build and reseeded, because it does not follow a push: ten cases, each with a trajectory, four of them resolved with a delay action, and four accounts. Both phases close at commit `41f744e` ([ERN-P15.60]), fingerprint `397ca55234709fe8`, with `/api/ready` returning `ready` and `verify-live` green. The fingerprint of the live build is that of the latest docs commit; the numbers here name the close commit.

Recipe as run: Fable wrote the brief from Ahmed's request, took his decisions and held the gate, three turns in all; Opus did everything else in one session on `main` in the main checkout: the spec from the brief, then the build tests first per item with one commit per coherent item, then the four-lens review with three refuters per finding (one round over both phases), the fixes each starting from an assertion that failed first, the whole verification chain, the deploy check by fingerprint, the demo redeploy and reseed, and these documents. Every stage ran as a single Opus agent rather than a workflow, because the permission classifier refuses further workflow launches here, the same refusal Phases 12 and 13 met.

### Phase 14: what was done about the delay, and who it went to (asked for on 12 September)

Ahmed worked the Phase 13 sheet himself and came back with three things: "hide the time line from case inputs page", "hide the updates section", and, in their place, a box for what was done to solve the delay and a Yes / No for whether it was escalated to the medical director. He also asked where the "Case management" block came from; it is his own decision B of Phase 8b, it stays behind "More to record", and this phase does not touch it.

`docs/specs/phase14-actions-and-escalation.md` is the spec: the labels, the ids, the two column names, the mirroring rule, the KPI rule, the export columns, the failing assertion each item starts from, a "contracts that stay" list, and a table of every test and document that referenced either section and what each becomes.

**What changed on the screen.** The server-rendered Timeline and the whole Updates section (the composer with its action tag, the Add button and the list) are off `/cases/[id]`, and the `Updates` chip leaves the "Jump to" strip, which is now Delay, Teams, Tests, Times, Resolve. In their place, first in the Resolve block above "Final disposition": **What was done to solve the delay**, one free-text box with dictation, a 1000-character cap and the standing "MRN only, no names" hint, and **Escalated to medical director**, Yes / No chips that clear on a second tap so a case nobody has answered stays unanswered. Both are editable on an open case and on a resolved one, and neither is ever required, not at "Open case" and not at "Mark resolved". Everything else on the sheet is what Phase 13 left.

**Why nothing is lost.** Three things read `CaseUpdate` rather than the case: the weekly deck's action figures, the board's "Updated 3h ago" and the QCH sheet's Comments column. So a save or a resolve that changes the box appends one `CaseUpdate` carrying that text, authored by whoever pressed Save, in the same transaction, with its own audit row, tagged `LEADERSHIP_ESCALATION` on the move to Yes and untagged otherwise. It only ever creates: a second text appends a second row and never touches the first, and the service has no `caseUpdate.update` and no `caseUpdate.delete`, which a test greps for. The timeline is still in the case summary sheet and on the handover sheet; the update list is still in the summary sheet, the workbook's Updates sheet, the QCH Comments column and the admin audit view; and `addCaseUpdate`, its permission check and its server action all stay.

**What changed in the figures.** "Actions documented" counts a case when it carries the escalation answer Yes, beside `medAdminInformedAt`, which has counted since Phase 8b, or a non-empty "what was done" text, which lands in the deck's seventh row, "Update without an action tag", because that is what it is. Two readings of the brief are recorded in the spec rather than left implicit: "any `CaseUpdate`" keeps Phase 8b review C2's exclusion of the app's own resolve, reopen, void and alert notes, and the two new export columns go on the per-case Cases sheet, because the Updates sheet is one row per update and the text reaches it as the mirrored row anyway.

**The data.** One additive migration, `20260912120000_delay_action_escalation`: `Case.delayActionTaken TEXT` and `Case.escalatedToMedicalDirector BOOLEAN`, both nullable, no default, no backfill, no existing row read or written. No taxonomy name is renamed, added or removed, and the permission matrix is what it was.

Verified on the lead database at the close: typecheck, lint, 1,152 unit and database tests in 66 files, the production build, 249 Playwright tests at 390 x 844 and 1280 x 800 (63 project skips), 15 in `tests/instance`, 7 in `tests/demo`, the app-role privilege guard, and `prisma migrate diff --from-config-datasource --to-schema prisma/schema.prisma --exit-code` reporting no difference once the migration is applied. Gate captures in `design/screens/phase14-*.png`.

**Reviewed with Phase 15.** The two phases were built a day apart on `main` and reviewed together in one four-lens round over the combined diff; the numbers, the six confirmed findings and the product point the round left with Ahmed are in the Phase 15 section above. One sentence of the spec was corrected by that round, and so were two here. The three questions this phase left open were closed inside Phase 15, one commit each, and Phase 14 goes live in the same build. Both phases close at commit `41f744e` ([ERN-P15.60]), fingerprint `397ca55234709fe8`, with `/api/ready` returning `ready` and `verify-live` green.
...
**What the review round found.** Four lenses over the whole diff (correctness, security, UX and documents, tests) with three refuters per finding: fourteen findings raised, five confirmed and one cheap note taken, each fixed with a test that failed first, and the nine refuted not carried. The shift had been moved behind the closed "More to record" while the form still filled it from the navigator's last shift and every save re-stamped that memory, so a wrong shift perpetuated itself unseen while the dashboard's `byShift` split and the export column read it; it is back in the identity block as a Morning / Evening / Night chip row, and the section keeps four answers and its promise that nothing recorded is behind a tap. A case resolved before this phase can be reopened into a state it cannot leave, so "Reopen case" now arms the same two-tap guard as Void, over a sentence naming the times that outcome will owe. The demo seed wrote four times and stopped, so its admitted and transferred cases were rows the app itself would refuse to resolve; every outcome's chain is backdated in flow order now, with the receiving facility. Both guides still described the old sheet and are rewritten against the running app, with the drift guard extended to catch the next one. The last fixed sleep in the demo kit is replaced by an expect-based wait. And the note: the closed "More to record" toggle reveals nothing recorded, so it stays off the paper.

Verified on the lead database at the close: typecheck, lint, 1,126 unit and database tests in 66 files, the production build, 237 Playwright tests at 390 x 844 and 1280 x 800 (63 project-specific skips), 15 in `tests/instance` and 7 in `tests/demo`, the app-role privilege guard, and `prisma migrate diff --from-config-datasource --to-schema prisma/schema.prisma --exit-code` reporting no difference. There is no migration in this phase. Gate captures in `design/screens/phase13-*.png`. The demo instance was redeployed by hand to the same build and reseeded, since it does not follow a push and its resolved cases predated the rule.

Recipe as run: Fable proposed the phase from Ahmed's five requests, took his confirmation and his one decision, and holds the gate; Opus did everything else in a single session on `main` in the main checkout, the spec first and then tests first per item, one commit per coherent item, then the four-lens review with refuters, the fixes, the whole chain, the deploy check, the demo reseed and these documents. Every stage ran as a single Opus agent rather than a workflow, because the permission classifier refuses further workflow launches here, the same refusal Phase 12 met. One lesson: the local chain ran Playwright at five workers and produced load-only timeouts on the heaviest dashboard specs, where CI's two workers are green, so the local chain runs at two now.
