...
- Reviewed and deployed. A four-lens review of the merged diff with three refuters per finding raised thirteen findings and confirmed seven, all fixed with a test that failed first: the demo seed's production refusal could never fire, because the database host is `db` on both copies, so it now reads the container's own `APP_URL`; the spec still described the redirect the code had replaced; the changelog named an export action that has never existed; the DEMO banner pushed the desktop rail's last link off an 800 px screen; and the export audit-row assertion read the newest row written by any spec and was flaky. The chain the phase left green: 1,063 unit and database tests in 65 files, 221 Playwright tests at 390 × 844 and 1280 × 800 with 63 project skips, 15 in `tests/instance`, the privilege guard, the build and no schema drift. The close commit is pushed to `main`; [ERN-P12.62] records its hash and fingerprint here.
