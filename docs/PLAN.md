...
- A twelve-agent read-only readiness audit of the repository, the host and the live app answered the question in 50 items (15 demo, 30 production, 5 after go-live, 10 already done), and Ahmed chose option B for the demo: a hosted copy on its own subdomain, its own database, its own secrets, blank SMTP. `docs/specs/phase12-go-live-readiness.md` is the build Claude owns: an instance banner so no one can mistake the demo for the real board, a forced password change on any account an admin creates or resets, one MRN-only hint under every free-text box, audit rows for the workbook download and the printed report, a bounded retry with an attempt count for an alert e-mail that failed, a demo seed of four accounts and ten invented patients that refuses to run against anything but a labelled instance, and the small hygiene items (a spreadsheet gitignore, a configurable login limit, three Dependabot bumps). A four-lens review of that spec put up twenty findings and confirmed nine, six of which changed it: the banner's white text moved off the 3.63:1 amber and gained `print-color-adjust`, a caller who must change their password is refused on the API rather than exempted from it, and the alert retry was bounded so it cannot outrun the worker's heartbeat.
...
- **12C, the documents.** This plan and the runbook stopped being true in a dozen places during Phases 7 to 11, and one of them, the runbook's "the repository is private", is a statement an operator would act on. Every one is corrected in place and dated, and each says what it used to say: the repository is public until Ahmed flips it, the worker is in send mode from the secondary domain's mailbox rather than log-only awaiting DKIM on `towardpcc.com`, the push monitor is live, the report header is an environment variable and not a settings row, the mark is the Envato cross and not a placeholder, Dependabot's npm updater has never opened a pull request, and the laptop backup mirror has never worked, so the chain is two copies and not three. The runbook's environment table gained the `REPORT_HEADER` row it never had, plus the two new Phase 12 variables, and a "Demo instance" section written from the spec. New: `docs/guide/nurse-quick-guide.md`, one printable page for a navigator that opens on "MRN only, never a name" and follows the shift from signing in to what to do when the site is down, and `docs/guide/demo-script.md`, a fifteen-minute presenter script in the order of `tests/demo/demo.spec.ts` with a hands-on handoff and the questions to expect.
- *(To be completed at the gate: what 12A shipped and the tests that proved it, what 12B provisioned and how production was shown untouched, the fingerprint and commit the phase left live, and how the demo itself went.)*
...
Ahmed asked, at the Phase 11 gate, what is left before staff can try this themselves and before real patients go on it, and for the part that is Claude's to be built. A twelve-agent read-only audit of the repository, the host and the live app answered the first half in 50 items ranked by what would go wrong first (15 demo, 30 production, 5 after go-live, 10 already done). Ahmed's decision on the demo was option B: a hosted copy on its own subdomain, a second Coolify application from the same repository with its own database, its own secrets, blank SMTP and no alert push URL, so that production is not touched by any of it.

What the audit found that this phase answers, in its order: nothing on the screen says which copy you are looking at, so a demo and the real board are the same pixels; the demo tooling would run against production if it were pointed at it; there is no seed that produces cases, so a hosted demo would open on an empty board; invented MRNs look real; the "MRN only, no names" hint is on one free-text box of five while the plan claims it is on all of them; the first password is never forced to change, so a password read out across a ward desk can stay in use for ever; an alert e-mail that fails twice is lost and both monitors stay green; an export or a printed report leaves no audit row; the login limit would turn away the sixth person in a minute at a demo behind one hospital NAT address; `.gitignore` would not stop a patient spreadsheet being committed; four Dependabot pull requests are open, one of which breaks the Docker build; several documents state things that are no longer true; and there is no user documentation at all.

`docs/specs/phase12-go-live-readiness.md` is the build, in three slices. **12A, the code:** the instance banner behind a new `INSTANCE_LABEL`, the must-change-password gate on any account an admin creates or resets, the MRN-only hint under all five free-text boxes, audit rows for the workbook download and the printed report, a bounded retry with an attempt count and a durable trace for an alert e-mail that failed, a demo seed of four accounts and ten invented patients shipped in the runner image beside `worker.js` that refuses to run against anything but a labelled instance, and the hygiene items (the spreadsheet gitignore with a `git check-ignore` proof that `src/` and `app/` are not caught by it, a configurable login limit, three safe Dependabot bumps applied locally so the pull requests close themselves). **12B, the infrastructure:** `demo-nav.towardpcc.com`, a Cloudflare record and a new Coolify project and application, every secret regenerated, SMTP and the push URL blank, seeded and verified, with production's fingerprint and environment key set proven unchanged. **12C, the documents:** the stale statements in this plan and the runbook, a one-page nurse quick guide, a fifteen-minute demo presenter script, the runbook's "Demo instance" section, and this section.

Nothing in this phase changes the workflow, the taxonomy, the permission matrix or any stored clinical field, and production with `INSTANCE_LABEL` unset behaves exactly as it did before.

**Three decisions this phase deliberately leaves open**, recorded here rather than taken quietly:

1. **Whether VIEWER keeps `export.xlsx`.** Leadership can download the whole workbook of MRNs today (`policy.ts` grants `export.xlsx` to SUPERVISOR, ADMIN and VIEWER). The audit's second half of C2 proposed removing it. That is a change to the locked permission matrix, so Phase 12 only makes the download auditable and leaves the grant alone.
...
Recipe as run: one workflow, Opus throughout. Fable wrote the brief and held the gate. Opus did everything else: the code map, the spec, a four-lens spec review with three refuters per finding (twenty findings, nine confirmed, six of which changed the spec) and its fixes, each held in place by `tests/unit/phase12-spec.test.ts`, which parses the document and checks it against `app/globals.css`, `docker-compose.production.yml` and `worker/alerts.ts` rather than trusting its prose; then the three slices in their own worktrees, tests first, the whole chain green before hand-back; then 12B on the host, then the demo.
