...
- Email: nodemailer over SMTP from `SMTP_*` env; template (text + minimal HTML): MRN, waiting time, primary reason, departments, link `${APP_URL}/cases/{id}`. Subject "ER Navigator: MRN {mrn} past {t}h". When `SMTP_HOST` is empty, log the message at info level instead of sending, and still set `emailSentAt = null`. Send failures: log, retry once after 30 s, never throw out of the cycle. Recipients are looked up each cycle (no caching).
