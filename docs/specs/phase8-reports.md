...
export type KpiCase = {
  ...
  registrationAt: Date; triageAt: Date | null; roomAt: Date | null; physicianAt: Date | null; decisionAt: Date | null
  ...
  transferRequestedAt: Date | null; transferAcceptedAt: Date | null; transportArrivedAt: Date | null; medAdminInformedAt: Date | null
...
export type Headline = { cases: number; measured: number; episodes: number; med: number | null; mean: number | null; min: number | null; max: number | null; atLeast10: number; atLeast12: number; longest: { id: string; mrn: string; hours: number } | null }
export function headline(cases, now): Headline                                  // med/mean null when n < MIN_N; min/max/longest whenever there is one; `measured` = cases with a computable stay
export function previousRange(all, range, now): { cases: KpiCase[]; asOf: Date } // the period of the same length before `inRange`'s; compute its headline with `headline(cases, asOf)`, NEVER with `now`
...
export type Completeness = { noReason: IdRow; openQuiet12h: IdRow; noDecision24h: IdRow; resolvedNoDisposition: IdRow; outOfOrder: IdRow; noStay: IdRow }
...
export function benchmark(kpi: AdaaKpi, value: number): Benchmark | null        // KPI 1–3 in minutes, 4–5 as a share 0..1; null for kpi6 (no benchmark)
export const ADAA_BENCHMARKS: Record<AdaaKpi, { unit; world; acceptable; improve; higherIsBetter } | null>
export function doorAt(c): Date                 // the Adaa door: earlier of registration and triage, used by KPI 1 AND KPI 5
export function leftAt(c): Date | null          // departed, else resolved, RESOLVED cases only (a reopened case is open everywhere)
export function kpi1Minutes(c): number | null   // door to physician
...
export function doorToDispositionHours(c): number | null  // door to left ED, resolved only
export const TREATED_BANDS  // 'Within 4 h', '4–6 h', '6–12 h', '12–24 h', '24–48 h', '48–72 h', '>72 h' (Adaa columns AH–AN; 4.0 is within, later bands are (min, max])
...
export function examToConsult(cases): StatRow[]       // n counts CONSULTS, ids the cases behind them; label the section accordingly
...
so it satisfies `KpiCase` above: add `triageAt, roomAt, physicianAt, decisionAt, resolvedAt, handoverAt,
transferRequestedAt, transferAcceptedAt, transportArrivedAt, medAdminInformedAt, wardCode, ctas, areaName,
updatesCount, lastUpdateAt` and `preliminaryAt` on investigations. Extend the fixture in `__tests__/aggregates.fixture.ts`
...
1. Headline tiles replace the current tile row: cases, episodes, median stay, mean stay,
   ...
   delta against `previousRange` shown as a small "+3 vs previous 30 d" line (compute the
   previous headline with the `asOf` it returns, never with `now`). Medians below MIN_N render
   "n<3" as everywhere; when `measured` is below `cases`, footnote "n cases have no computable
   stay (registration in the future)".
...
Statistics" block: rows CTAS 1..5, then "CTAS not recorded" when `adaaSummary` returns an
'unknown' row, then Total; columns Total patients, Door to Doctor (total
...
the range, status filter), how many rows lack a CTAS, what is blank until recorded, that KPI 6
here is DAMA only (LAMA is not a disposition yet, so it is a lower bound), that the door is the
earlier of registration and triage for KPI 1 and KPI 5 alike, that the "within 1 hour"
admission band stops at 1 h 00 min where the official formula reaches 1 h 00.6 min, the paste
instruction ("select A2:T… ,
