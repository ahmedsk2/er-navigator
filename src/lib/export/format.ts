 ...
 * an empty string for anything missing, or a blank cell for a missing hours figure (the prototype's `fmtDT` renders a dash on screen; a spreadsheet wants a blank cell so
...
/**
 * A numeric cell rounded to two decimals, or null for a truly blank cell. Not an empty string: to
 * Excel that is a text cell, which a chart plots as zero and COUNTA counts, while null writes no
 * value at all. The writer gives every number the "0.00" format.
 */
export function fmtHours2(hours: number | null | undefined): number | null {
  return hours == null || Number.isNaN(hours) ? null : Math.round(hours * 100) / 100
}
